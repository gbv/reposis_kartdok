...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="user" mode="email">
    ...
    <subject>
      ...
      -->
      <xsl:value-of select="i18n:translate('kartdok.email.newauthor.subject')" />
    ...
    <body>
      ...
      <!-- START kartdok adjustments -->
      <xsl:value-of select="i18n:translate('kartdok.email.newauthor.bodytext')" />
      <!-- END kartdok adjustments -->
      <xsl:value-of select="$newline" />
      ...
      <xsl:value-of select="
        concat(
          '&lt;',
          $ServletsBaseURL, 'MirSelfRegistrationServlet?action=verify&amp;user=', @name, '&amp;realm=', @realm, '&amp;token=', attributes/attribute[@name='mailtoken']/@value,
          '&gt;'
        )" />
      <!--
      <xsl:value-of select="$newline" />
