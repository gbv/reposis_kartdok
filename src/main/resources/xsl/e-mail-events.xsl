...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mcrmods="xalan://org.mycore.mods.classification.MCRMODSClassificationSupport"
  ...
  <xsl:template match="user" mode="output">
    ...
    <xsl:value-of select="concat(i18n:translate('kartdok.email.institute'),' ',$instNames,$newline)" />
    <xsl:value-of select="concat(i18n:translate('kartdok.email.eMail'),' ',eMail,$newline)" />
    <xsl:value-of select="concat(i18n:translate('kartdok.email.link'),' ',$ServletsBaseURL,'MCRUserServlet?action=show&amp;id=',@name,'@',@realm,$newline)" />
    <!-- END kartdok adjustments -->
    <xsl:value-of select="@id" />
  </xsl:template>
  ...
  <xsl:template match="mycoreobject" mode="output">
    <xsl:if test="./metadata/def.modsContainer/modsContainer/mods:mods/mods:titleInfo/mods:title">
      ...
      -->
      <xsl:value-of select="concat(i18n:translate('kartdok.email.title'),' ',./metadata/def.modsContainer/modsContainer/mods:mods/mods:titleInfo/mods:title[1],$newline)" />
      <!-- END kartdok adjustments -->
    </xsl:if>
    <xsl:if test="./metadata/def.modsContainer/modsContainer/mods:mods/mods:name[mods:role/mods:roleTerm='aut']">
      ...
      -->
      <xsl:value-of select="concat(i18n:translate('kartdok.email.authors'),' ',$authors,$newline)" />
      <!-- END kartdok adjustments -->
    </xsl:if>
    <!-- START kartdok adjustments -->
    <!--
    ...
    -->
    <xsl:value-of select="concat(i18n:translate('kartdok.email.link'),' &lt;',$WebApplicationBaseURL,'receive/',@ID, '&gt;')" />
    <!-- END kartdok adjustments -->
  </xsl:template>
